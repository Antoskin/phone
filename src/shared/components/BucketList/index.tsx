    ...
    <div className='space-y-10 lg:w-1/2 mx-auto'>
      {products.map((product: IProduct) => (
        <div key={product.id} className='space-y-4 border-b border-gray-300 pb-5 last:border-b-0'>
          <h3 className='text-2xl font-bold'>{product.slug}</h3>
          <p>{product.description}</p>
          <div className='flex justify-end gap-2'> <Button type='button' onClick={() => pathTo(Number(product.id))}> <FaEye /> </Button> <Button type='button' onClick={() => removeFromBucket(Number(product.id))}> <FaTrash /> </Button> </div> </div> ))} </div> )
}
